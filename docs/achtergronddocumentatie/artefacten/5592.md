...
Zowel een `klantcontact` als een `inerneTaak` zijn altijd gekoppeld aan een één of meer `actoren`. Dat betekent dat een `actor` die aan een `klantcontact` of aan een `interneTaak` gekoppeld moet worden als bestaat voordat het `klantcontact` of de  `interneTaak` wordt geregistreerd. 
Deze registratiefunvtie heeft dus met name het karakter van een "beheer-functie".  

Er zijn 3 typen actoren. De resource `actor` is polymorfisch. Bij de schrijf-actie is de waarde van de propertie `soortActor` bepalend voor de structuur van de resource.  
