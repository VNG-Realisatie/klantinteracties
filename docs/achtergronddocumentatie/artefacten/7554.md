...
Aangezien de `partij-identifcator` pas kan worden vastgelegd als de ID van de `Partij` bekend is is er niet te ontkomen aan tijdelijke inconsistentie. 
...
- Indien de `partij` wel is vastgelegd (en er dus een responsecode '201' met responsebody is ontvangen, maar de `partij-identifcator` kan niet worden vastgelegd door de provider dient de consumer zelf de zojuist aangemaakte partij te verwijderen.
