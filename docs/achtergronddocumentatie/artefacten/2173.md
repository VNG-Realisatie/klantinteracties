...
Indien de medewerker is ingelogd kan de koppeling tussen `actor` en `klantcontact` door de consumerapplicatie zonder tussenkomst van de gebruiker gelegd worden. Daarvoor is het nodig dat de *id* van de medewerker als `actor` bekend is in de consumerapplicatie. 

Eventuele andere medewerkers die betrokken zijn bij het `klantcontact` worden gekoppeld middel [F6542 - Koppel Actor aan Klantcontact](./6542.md).
...
Het vastleggen van de `betrokkeneBijKlantcontact` kan, zoals eerder beschreven bij [Functionaliteit]($F2173%20-%20Registreer%20klantcontact/Functionaliteit)
